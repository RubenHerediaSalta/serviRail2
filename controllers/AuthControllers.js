...
const JWT_KEY = "fiverrsupersecretkey";
...
export const addFavorite = async (req, res, next) => {
  try {
    ...
    if (req.userId && gigId) {
      ...
      return res.status(200).send("Gig added to favorites successfully.");
    }
...
export const removeFavorite = async (req, res, next) => {
  try {
    ...
    if (req.userId && gigId) {
      ...
      return res.status(200).send("Gig removed from favorites successfully.");
    }
    return res.status(400).send("userId and gigId are required.");
  } catch (err) {
    console.log(err);
